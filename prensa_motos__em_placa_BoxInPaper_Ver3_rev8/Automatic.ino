void automatico()
{
  ...
  while (!TstBit(PINC, 0) && !TstBit(PINC, 1))
  {
    ...
    if (!ligaPrensa) { //                    botão do alimentador prescionado pelo tempo da variavel pré selecionada
      ...
      if (!TstBit(PIND, 4) && !btAlimentador)
      { //se o botão é prescionado e a variavel é LOW
        btAlimentador = HIGH;
        tempoPressionado = millis();
      }
      if (TstBit(PIND, 4) && btAlimentador)
        btAlimentador = LOW; // se o botão for solto variavel passa a ser LOW
      if (!TstBit(PIND, 4) && btAlimentador && millis() - tempoPressionado >= tempoZerar)
      {
