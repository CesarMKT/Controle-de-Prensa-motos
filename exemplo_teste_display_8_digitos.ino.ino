...
void setLEDs (unsigned long  number) {
  ...
  for (unsigned int i = 0; i < Segments * digitsPerSegment; i++) {
    ...
    if ( i % digitsPerSegment + positionOffset == 3 ) lc.setDigit(Segments - i / digitsPerSegment - 1, i % digitsPerSegment + positionOffset, num, true);
    else if ( i % digitsPerSegment + positionOffset == 6 ) lc.setDigit(Segments - i / digitsPerSegment - 1, i % digitsPerSegment + positionOffset, num, true);
    else lc.setDigit(Segments - i / digitsPerSegment - 1, i % digitsPerSegment + positionOffset, num, false);
  }
