...
void setup()
{
  ...
  if (marcador > 998)
    marcador = 2; // verifica se a nova posição e maior que o final da memoria - se for zera marcador
  if (marcador < 2)
    marcador = 2;                     // na primeira utilização do programa o marcador deve ser definido no segundo emdereço para iniciar a contagem
  contador = EEPROMReadInt(marcador); // memoriza contador na variavel
  if (contador != EEPROMReadInt(marcador - 2))
    marcador = marcador + 2; // se o contador == ao anterior evita reescrever o marcador senão
                             //cria nova posição para gravar contagem do ciclo
  if (marcador > 998)
    marcador = 2; // verifica se a nova posição e maior que o final da memoria - se for zera marcador
  if (contador != EEPROMReadInt(marcador))
    EEPROMWriteInt(marcador, contador); // verifica se o valor é diferente no endereço, se for atualiza
  if (marcador != EEPROMReadInt(0))
    EEPROMWriteInt(0, marcador); // por ultimo grava novo endereço na memoria do novo ciclo caso for diferente
...
void loop()
{
  ...
  if (!TstBit(PIND, BotaoManual))
  { // se chave for trocada de posicao
    Manual();
  }
  else
  {
...
void Manual()
{
  ...
  {                             //Verifica se Botão foi pressionado
    ...
    if (estadoPinca)
      SetBit(PORTB, RelePincaA); //muda estado da porta diretamente no registrador
    else
      SetBit(PORTB, RelePincaF); //conforme condição da variavel
    while (!TstBit(PIND, BotaoPinca))
    {                // entra no looping enquanto o botão for pressionado
  ...
  // Morça Abre
  if (!TstBit(PIND, BotaoMorca))
  { // idem Pinca Abre
    estadoMorca = !estadoMorca;
    if (estadoMorca)
      SetBit(PORTB, ReleMorcaA);
    else
      SetBit(PORTB, ReleMorcaF);
  ...
  {                             // verifica o estado do botão para entrar no condicional
    estadoMotor = !estadoMotor; // altera  o estado da variavel lidado/ desligado
    if (estadoMotor)
      SetBit(PORTB, ReleMotorLiga); //Liga rele Motor conforme variavel HIGH
    else
      ClrBit(PORTB, ReleMotorLiga); //Desliga rele do motor se LOW
...
void Automatico()
{
  ...
  if (!TstBit(PIND, BotaoIniciar))
  { // aguarda botão iniciar ser pressionado
