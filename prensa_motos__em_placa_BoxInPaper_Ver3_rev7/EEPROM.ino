...
void EEPROMIniciar()
{
  ...
  if (marcador > 990)
    marcador = 4; // verifica se a nova posição e maior que o final da memoria - se for zera marcador
  if (marcador < 4)
    marcador = 4;                      // na primeira utilização do programa o marcador deve ser definido no segundo emdereço para iniciar a contagem
  contador = EEPROMReadLong(marcador); // memoriza contador na variavel
  if (contador != EEPROMReadLong(marcador - 4))
    marcador = marcador + 4; // se o contador == ao anterior evita reescrever o marcador senão
  ...
  if (marcador != EEPROMReadLong(0))
    EEPROMWriteLong(0, marcador); // por ultimo grava novo endereço na memoria do novo ciclo caso for diferente
