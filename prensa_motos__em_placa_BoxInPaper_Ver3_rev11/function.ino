...
{
  contadorAuto++;
  if (contadorAuto > 32000) // reduz o valor para não estourar a memoria
    contadorAuto = 30000;
...
{
  if (ligaPrensa)
    SetBit(PORTB, RelePrensa);
  else
    ClrBit(PORTB, RelePrensa);
  if (ligaAlimentador)
    SetBit(PORTB, ReleAlimentador);
  else
  ...
  if (ligaLuz)
    SetBit(PORTB, ReleSinaliza);
  else
    ClrBit(PORTB, ReleSinaliza);

  if (TstBit(PIND, BtBimanualDireito) && !btDireito)
    btDireito = HIGH;
  if (TstBit(PIND, BtBimanualEsquerdo) && !btEsquerdo)
    btEsquerdo = HIGH;
  if (pot1 != EEPROMReadInt(996))
    EEPROMWriteInt(996, pot1);
} //end atualizar
//_________________________________________________________________________//
